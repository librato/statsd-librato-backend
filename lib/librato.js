...
// Set global prefix to identify data source.
var globalPrefix = "";
...
exports.init = function librato_init(startup_time, config, events, logger)
{
  ...
  if (config.librato) {
    ...
    if (globalPrefix !== "") {
      countersNamespace.push(globalPrefix);
      gaugesNamespace.push(globalPrefix);
      measureTimeNamespace.push(globalPrefix);
    }

    if (counters !== "") {
      countersNamespace.push(counters);
    }
    if (gauges !== "") {
      gaugesNamespace.push(gauges);
    }
    if (measureTime !== "") {
      measureTimeNamespace.push(measureTime);
    }
  } else {
      countersNamespace = ['stats'];
      gaugesNamespace = ['stats', 'gauges'];
      measureTimeNamespace = ['stats', 'timers'];
  }

  // -- end of code from graphite --
