...
var flush_stats = function librato_flush(ts, metrics)
{
  ...
  var addMeasure = function add_measure(mType, measure, countStat) {
    ...
    // check if the metric name has a source value included before a '/' separator, and
    ...
    if (metricName.indexOf('/') > 0 && metricName.indexOf('/') < (metricName.length-1)) {
      measure.source = sanitize_name(metricName.split('/',1)[0]);
      measure.name = sanitize_name(metricName.slice(measure.source.length + 1));
    }
    else {
